...
function calculateFixedCharge(units) {
  if (units > 60) {
    if (units > 180) return config.prices.fixed.blockFive;
    if (units >= 91) return config.prices.fixed.blockFour;
    if (units >= 61) return config.prices.fixed.blockThree;
  }
...
export { calculateIncome, calculateExpense };
